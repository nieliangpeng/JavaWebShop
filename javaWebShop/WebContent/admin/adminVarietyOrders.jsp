...
<br />
		...
		<div id="lqy_product_showpage">
			...
			<c:if test="${!(pageCount.isFirst)}">
  				&nbsp;<a href='${path_}/showVarietyOrdersInAdmin.action?status=${status}&pageNum=1'><button style="border-radius: 6px;color: green;">首页</button></a>&nbsp;
  				&nbsp;<a href='${path_}/showVarietyOrdersInAdmin.action?status=${status}&pageNum=${pageCount.showPage-1}'><button style="border-radius: 6px;color: green;">上一页</button></a>&nbsp;
			</c:if>
			<c:if test="${pageCount.isLast}">
  				&nbsp;<font color="green">尾页</font>&nbsp;&nbsp;
			</c:if>
			<c:if test="${!(pageCount.isLast)}">
	  			&nbsp;<a href='${path_}/showVarietyOrdersInAdmin.action?status=${status}&pageNum=${pageCount.showPage+1}'><button style="border-radius: 6px;color: green;">下一页</button></a>&nbsp;
	  			&nbsp;<a href='${path_}/showVarietyOrdersInAdmin.action?status=${status}&pageNum=${pageCount.count}'><button style="border-radius: 6px;color: green;">尾页</button></a>&nbsp;
			</c:if>
