...
<body>
	...
	<center>
		<font color="green" style="font-size: 34px;font-weight:bolder;">搜索'${searchFoodName}'的结果</font><br/><br/>
			...
			<c:forEach items="${foodList}" var="food" >
				<tr style="text-align: center;">
					...
					<td height="27px" style="border-color: #F6F6F6;">
						<a href="${path_}/searchFoodDetail.action?id=${food.id}&searchFoodName=${searchFoodName}"><button style="border-radius: 6px;color: green;">详细</button></a>
						<a href="${path_}/removeSearchFood.action?id=${food.id}&foodName=${food.foodName}&searchFoodName=${searchFoodName}"><button style="border-radius: 6px;color: red;">删除</button></a>
		...
		<div id="lqy_product_showpage">
			...
			<c:if test="${!(pageCount.isFirst)}">
  				&nbsp;<a href='${path_}/searchFoodSubmit.action?pageNum=1&searchFoodName=${searchFoodName}'><button style="border-radius: 6px;color: green;">首页</button></a>&nbsp;
  				&nbsp;<a href='${path_}/searchFoodSubmit.action?pageNum=${pageCount.showPage-1}&searchFoodName=${searchFoodName}'><button style="border-radius: 6px;color: green;">上一页</button></a>&nbsp;
			</c:if>
			<c:if test="${pageCount.isLast}">
  				&nbsp;<font color="green">尾页</font>&nbsp;&nbsp;
			</c:if>
			<c:if test="${!(pageCount.isLast)}">
	  			&nbsp;<a href='${path_}/searchFoodSubmit.action?&pageNum=${pageCount.showPage+1}&searchFoodName=${searchFoodName}'><button style="border-radius: 6px;color: green;">下一页</button></a>&nbsp;
	  			&nbsp;<a href='${path_}/searchFoodSubmit.action?&pageNum=${pageCount.count}&searchFoodName=${searchFoodName}'><button style="border-radius: 6px;color: green;">尾页</button></a>&nbsp;
			</c:if>
